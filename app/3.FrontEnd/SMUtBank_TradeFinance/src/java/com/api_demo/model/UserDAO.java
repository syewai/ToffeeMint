...
public class UserDAO {
    ...
    public User retrieve(String username) {
        try {
            conn = ConnectionFactory.getConnection("toffeemint");
            pstate = conn.prepareStatement("SELECT username, password, usertype FROM user WHERE username = '" + username + "'");
            rs = pstate.executeQuery();
            while (rs.next()) {
                if (rs.getString(1) != null) {
...
//            System.out.println("login:" + lastLoginTimeStamp + username);
    ...
    public Boolean login(String username, String password, String usertype) {
        ...
        if(user != null){
            ...
            String storedUserType = user.getUsertype();
            if (storedPassword.equals(password) && storedUserType.equals(usertype)) {
